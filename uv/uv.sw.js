"use strict";(()=>{if (!self.Ultraviolet) {
    console.error('Ultraviolet is not initialized. Ensure uv.bundle.js is loaded correctly.');
    // Attempt to load uv.bundle.js dynamically if not already loaded
    importScripts('/uv/uv.bundle.js');
    if (!self.Ultraviolet) {
        throw new Error('Failed to initialize Ultraviolet. Ensure uv.bundle.js is accessible.');
    }
}

// Replace localStorage with a service worker-compatible storage mechanism
const uvStorage = {
    async getItem(key) {
        const cache = await caches.open('uv-storage');
...
// Ensure bare-mux workerPath is set in uvStorage
(async () => {
    const workerPath = await uvStorage.getItem('bare-mux-workerPath');
    if (!workerPath) {
        await uvStorage.setItem('bare-mux-workerPath', '/uv/bare/');
    }
})();

// Wrap UVServiceWorker initialization in a try-catch block
try {
...
`,r.body+=t.js.rewrite(await c.text()),r.body+=`
    ...
    console.error('Error initializing UVServiceWorker:', error);
}
})();
