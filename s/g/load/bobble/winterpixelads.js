...
class WinterpixelAds { 
	...
	init(adBreak, adConfig, driver) {
		...
		if(this.driver == "winterpixel") {
			...
			PokiSDK.init().then(() => {
				...
				console.log("[PokiSDK] measureEvent(loading, poki_sdk_loaded, poki_sdk_loaded)");
				PokiSDK.measure("loading", "poki_sdk_loaded", "poki_sdk_loaded");
				// fire your function to continue to game
				PokiSDK.gameLoadingStart();
			}).catch(() => {
				console.log("Initialized, but the user likely has adblock");
				console.log("[PokiSDK] measureEvent(loading, poki_sdk_loaded, poki_sdk_loaded_adblock)");
				PokiSDK.measure("loading", "poki_sdk_loaded", "poki_sdk_loaded_adblock");
				// fire your function to continue to game
			});
		}
	};
	...
	measureEvent(category, action, label) {
		console.log("[PokiSDK] js measure_event(" + category + ", " + action + ", " + label + ")");
		if (this.driver == "poki") {
			if (PokiSDK) {
				if (PokiSDK.measure) {
					PokiSDK.measure(category, action, label);
				} else {
					console.log("[PokiSDK] js measure_event(" + category + ", " + action + ", " + label + "), but no PokiSDK.measure");
				}
			} else {
				console.log("[PokiSDK] js measure_event(" + category + ", " + action + ", " + label + "), but no PokiSDK");
			}
		}
	}
};
