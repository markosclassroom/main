...
//Make sure to check out https://monkeytype.com/

const SELECTOR_FOCUSABLE = `a[href], area[href], button:not([disabled]):not([aria-hidden]), input:not([disabled]):not([aria-hidden]):not([type="hidden"]), select:not([disabled]):not([aria-hidden]), textarea:not([disabled]):not([aria-hidden]), embed, iframe, object, [contenteditable], [tabindex]:not([tabindex^="-"]) `;
...
$("#wordsInput").focusout(() => {
  $('#fw').css({'opacity': '1',});
  $('#centerContent').css({'filter':'blur(2.5px)', 'transition': '0.5s'})
});

$("#wordsInput").focusin(() => {
  $('#fw').css({'opacity': '0'});
  $('#centerContent').css({'filter':'blur(0px)', 'transition': '0.5s'})
});
    ...
    let customText = "The quick brown fox jumped over the lazy dog, while the vigilant white cat leaped over the mouse.";
