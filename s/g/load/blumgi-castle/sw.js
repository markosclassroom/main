...
async function CreateCacheFromFileList(cacheName, fileList, bypassCache) {
    ...
    try {
        return await Promise.all(responses.map((response,
            i) => cache.put(fileList[i], response)))
    } catch (err) {
...
async function UpdateCheck(isFirst) {
    try {
        ...
        if (!response.ok) throw new Error(OFFLINE_DATA_FILE + " responded with " + response.status + " " + response.statusText);
        ...
        await CreateCacheFromFileList(currentCacheName,
            fileList, !isFirst);
        const isUpdatePending = await IsUpdatePending();
        if (isUpdatePending) {
...
async function HandleFetch(event, doUpdateCheck) {
    ...
    const lazyLoadList =
        result[1];
    if (IsUrlInLazyLoadList(event.request.url, lazyLoadList)) try {
