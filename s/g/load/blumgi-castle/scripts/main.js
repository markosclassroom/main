...
window.DOMElementHandler = class extends self.DOMHandler {
    ...
    _OnSetVisible(a) {
        this._autoAttach && (this._elementMap.get(a.elementId).style.display =
            a.isVisible ? "" : "none")
    }
    _OnUpdatePosition(a) {
        if (this._autoAttach) {
...
window.RuntimeInterface = class a {
    constructor(b) {
        ...
        "html5" !== this._exportType ||
            window.isSecureContext || console.warn("[Construct] Warning: the browser indicates this is not a secure context. Some features may be unavailable. Use secure (HTTPS) hosting to ensure all features are available.");
        ...
        "cordova" === this._exportType ? document.addEventListener("deviceready", () => this._Init(b)) :
            this._Init(b)
    }
    Release() {
    ...
    async _Init(b) {
        ...
        b.runtimeBaseUrl ? this._runtimeBaseUrl = b.runtimeBaseUrl : (c = location.origin, this._runtimeBaseUrl = ("null" === c ? "file:///" : c) + location.pathname, c = this._runtimeBaseUrl.lastIndexOf("/"), -1 !== c && (this._runtimeBaseUrl =
            this._runtimeBaseUrl.substr(0, c + 1)));
    ...
    _GetWorkerURL(b) {
        b = this._workerScriptURLs.hasOwnProperty(b) ? this._workerScriptURLs[b] : b.endsWith("/workermain.js") && this._workerScriptURLs.hasOwnProperty("workermain.js") ? this._workerScriptURLs["workermain.js"] : "playable-ad" === this._exportType && this._localFileBlobs.hasOwnProperty(b.toLowerCase()) ? this._localFileBlobs[b.toLowerCase()] :
            b;
    ...
    async _InitDOM(b, c) {
        ...
        for (let [g, h] of f)
            ...
            } else if ("scriptsInEvents.js" === g || g.endsWith("/scriptsInEvents.js")) h =
                await this._MaybeGetCordovaScriptURL(h), await AddScript(h);
    ...
    _ParseDataURI(b) {
        var c = b.indexOf(",");
        if (0 >
            c) throw new URIError("expected comma in data: uri");
...
RuntimeInterface$jscomp$1.AddDOMHandlerClass(class extends self.DOMHandler {
    constructor(a) {
        ...
        a.AddRuntimeComponentMessageHandler("runtime", "before-start-ticking", () =>
    ...
    Attach() {
        ...
        "undefined" !== typeof PointerEvent ?
            (window.addEventListener("pointerdown", b => {
                ...
                b)), window.addEventListener("pointerup", b => this._OnPointerEvent("pointerup", b)), window.addEventListener("pointercancel", b => this._OnPointerEvent("pointercancel", b))) : (window.addEventListener("mousedown", b => {
                this._HandlePointerDownFocus(b);
    ...
    _OnAndroidVirtualKeyboardChange(a, b) {
        ...
        if (0 < b) {
            ...
            c && (c = c.getBoundingClientRect(), a = (c.top + c.bottom) /
                2 - (a - b) / 2, a > b && (a = b), 0 > a && (a = 0), 0 < a && (document.body.style.transform = `translateY(${-a}px)`, this._vkTranslateYOffset = a))
        }
    }
    ...
    _TrySetTargetOrientation() {
        ...
        if (screen.orientation && screen.orientation.lock) screen.orientation.lock(a).catch(b => console.warn("[Construct] Failed to lock orientation: ", b));
            ...
            let b = !1;
            screen.lockOrientation ? b = screen.lockOrientation(a) :
                screen.webkitLockOrientation ? b = screen.webkitLockOrientation(a) : screen.mozLockOrientation ? b = screen.mozLockOrientation(a) : screen.msLockOrientation && (b = screen.msLockOrientation(a));
    ...
    _OnPointerEvent(a, b) {
        if (!this._isExportToVideo) {
            ...
            "mouse" === b.pointerType &&
                (c = "mousemove", "pointerdown" === a ? c = "mousedown" : "pointerup" === a && (c = "mouseup"), this._OnMouseEvent(c, b, DISPATCH_SCRIPT_ONLY), this._mousePointerLastButtons = b.buttons)
        }
    }
    ...
    _IsElementCanvasOrDocument(a) {
        return !a || a === document || a === window || a === document.body || "canvas" ===
    ...
    _OnUpdateCanvasSize(a) {
        ...
        b.IsExportingToVideo() || (b = b.GetCanvas(), b.style.width =
            a.styleWidth + "px", b.style.height = a.styleHeight + "px", b.style.marginLeft = a.marginLeft + "px", b.style.marginTop = a.marginTop + "px", this._isFirstSizeUpdate && (b.style.display = "", this._isFirstSizeUpdate = !1))
    }
    _OnInvokeDownload(a) {
        const b = a.url;
        a = a.filename;
    ...
    async _OnRasterSvgImage(a) {
        ...
        a = await self.C3_RasterSvgImageBlob(a.blob, a.imageWidth, a.imageHeight, a.surfaceWidth,
            a.surfaceHeight);
    ...
    _OnDebugHighlight(a) {
        if (a.show) {
            this._debugHighlightElem || (this._debugHighlightElem =
                document.createElement("div"), this._debugHighlightElem.id = "inspectOutline", document.body.appendChild(this._debugHighlightElem));
    ...
    _OnWrapperMessage(a) {
        "entered-fullscreen" === a ? (RuntimeInterface$jscomp$1._SetWrapperIsFullscreenFlag(!0), this._OnFullscreenChange()) : "exited-fullscreen" === a ? (RuntimeInterface$jscomp$1._SetWrapperIsFullscreenFlag(!1), this._OnFullscreenChange()) : console.warn("Unknown wrapper message: ", a)
    }
...
self.AudioDOMHandler = class extends self.DOMHandler {
    ...
    async _CreateAudioContext(a) {
        ...
        try {
            ...
            console.error("[Construct] Preloading sounds failed: ",
                c)
        }
        return {
    ...
    async _GetAudioInstance(a, b, c, d, e) {
        for (const f of this._audioInstances)
            if (f.GetUrl() === b && (f.CanBeRecycled() || e)) return f.SetTag(d),
                f;
    ...
    Tick() {
        ...
        this.PostToRuntime("state", {
            ...
            outputLatency: this._audioContext.outputLatency || 0,
            audioInstances: b,
            analysers: [...this._analysers].map(c =>
                c.GetData())
        });
    ...
    _OnTick(a) {
        ...
        for (const c of a.instPans) {
            a = c.uid;
            for (const d of this._audioInstances) d.GetUID() ===
                a && d.SetPanXYA(c.x, c.y, c.angle)
        }
    }
    async _AddEffect(a) {
        var b = a.type;
        ...
        if ("filter" === b) d = new self.C3AudioFilterFX(this, ...d);
            ...
            d._SetBufferInfo(a.bufferOriginalUrl, a.bufferUrl,
                a.bufferType)
        ...
        else if ("analyser" === b) d = new self.C3AudioAnalyserFX(this,
            ...d);
    ...
    async _OnLoadState(a) {
        ...
        if (3 !== b)
            for (var c of this._audioInstances) c.IsMusic() && 1 === b || (c.IsMusic() || 2 !== b) && c.Stop();
        for (const d of this._effects.values())
            for (const e of d) e.Release();
        this._effects.clear();
        this._timeScale = a.timeScale;
        this._gameTime = a.gameTime;
        c = a.listenerPos;
    ...
    _OnMicrophoneStream(a, b) {
        ...
        this._microphoneSource =
            this._audioContext.createMediaStreamSource(a);
    ...
    async _OnOfflineRenderAudio(a) {
        try {
            ...
            this._hasStartedOfflineRender ? this._audioContext.resume() : (this._audioContext.startRendering().then(c =>
                this._OnOfflineRenderCompleted(c)).catch(c => this._OnOfflineRenderError(c)), this._hasStartedOfflineRender = !0);
            await b
        } catch (b) {
    ...
    static LinearToDb(a) {
        return self.AudioDOMHandler.LinearToDbNoCap(Math.max(Math.min(a,
            1), 0))
    }
    static e4(a, b) {
...
self.C3AudioInstance = class {
    ...
    SetPan(a, b, c, d, e, f) {
        this._isPannerEnabled && (this.SetPanXYA(a, b, c), a = self.AudioDOMHandler.ToDegrees, this._pannerNode.coneInnerAngle =
            a(d), this._pannerNode.coneOuterAngle = a(e), this._pannerNode.coneOuterGain = f)
    }
    SetPanXYA(a, b, c) {
...
self.C3WebAudioInstance = class extends self.C3AudioInstance {
    ...
    _OnEnded(a) {
        this._isPaused ||
            this._resumeMe || a.target !== this._activeSource || (this._isStopped = this._hasPlaybackEnded = !0, this._instUid = -1, this._ReleaseBufferSource(), this._audioDomHandler.PostTrigger("ended", this._tag, this._aiId))
    }
    HasEnded() {
    ...
    GetPlaybackTime() {
        let a;
        a = this._isPaused ? this._resumePosition : this._playFromSeekPos + (this.GetCurrentTime() -
    ...
    Seek(a) {
        this._isStopped || this.HasEnded() || (this._isPaused ?
            this._resumePosition = a : (this.Pause(), this._resumePosition = a, this.Resume()))
    }
    GetResumePosition() {
...
self.RuntimeInterface.AddDOMHandlerClass(class extends self.DOMElementHandler {
    ...
    CreateElement(a, b) {
        ...
        c.addEventListener("pointermove", StopPropagation);
        c.addEventListener("pointerrawupdate", StopPropagation);
        c.addEventListener("pointerup", StopPropagation);
        c.addEventListener("mousedown", StopPropagation);
        c.addEventListener("mouseup", StopPropagation);
        c.addEventListener("keydown", StopKeyPropagation);
        c.addEventListener("keyup", StopKeyPropagation);
        c.addEventListener("click", e => {
            e.stopPropagation();
            this._PostToRuntimeElementMaybeSync("click", a)
        });
        c.addEventListener("dblclick", e => {
            e.stopPropagation();
            this._PostToRuntimeElementMaybeSync("dblclick", a)
        });
        c.addEventListener("input",
            () => this.PostToRuntimeElement("change", a, {
                text: c.value
            }));
        b.id && (c.id = b.id);
        b.className && (c.className = b.className);
        this.UpdateState(c, b);
        return c
    }
    UpdateState(a, b) {
        a.value = b.text;
        ...
        a.readOnly = b.isReadOnly;
        a.spellcheck = b.spellCheck;
        b = b.maxLength;
        0 > b ? a.removeAttribute("maxlength") : a.setAttribute("maxlength", b)
    }
    _OnScrollToBottom(a) {
