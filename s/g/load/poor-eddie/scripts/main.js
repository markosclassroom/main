...
window.RuntimeInterface = class a {
    ...
    async _Init(b) {
        ...
        b.runtimeBaseUrl ? this._runtimeBaseUrl =
            b.runtimeBaseUrl : (c = location.origin, this._runtimeBaseUrl = ("null" === c ? "file:///" : c) + location.pathname, c = this._runtimeBaseUrl.lastIndexOf("/"), -1 !== c && (this._runtimeBaseUrl = this._runtimeBaseUrl.substr(0, c + 1)));
    ...
    _GetWorkerURL(b) {
        b = this._workerScriptURLs.hasOwnProperty(b) ? this._workerScriptURLs[b] : b.endsWith("/workermain.js") &&
            this._workerScriptURLs.hasOwnProperty("workermain.js") ? this._workerScriptURLs["workermain.js"] : "playable-ad" === this._exportType && this._localFileBlobs.hasOwnProperty(b) ? this._localFileBlobs[b] : b;
        ...
        if ("cordova" === this._exportType)
            for (let h = 0, k = b.projectScripts.length; h < k; ++h) {
                const l = b.projectScripts[h],
                    m = l[0];
                if (m === b.mainProjectScript || "scriptsInEvents.js" === m || m.endsWith("/scriptsInEvents.js")) l[1] =
    ...
    _ReportProjectMainScriptError(b, c) {
        ...
        console.error(`[Preview] Failed to load project main script (${b}): `,
            c);
    ...
    _OnMessageFromRuntime(b) {
        const c = b.type;
        if ("event" ===
            c) return this._OnEventFromRuntime(b);
    ...
    async _MaybeGetCordovaScriptURL(b) {
        return "cordova" === this._exportType && (b.startsWith("file:") ||
            this._isFileProtocol && this.IsRelativeURL(b)) ? (b.startsWith(this._runtimeBaseUrl) && (b = b.substr(this._runtimeBaseUrl.length)), b = await this.CordovaFetchLocalFileAsArrayBuffer(b), b = new Blob([b], {
            type: "application/javascript"
    ...
    _RequestPermission(b) {
        const c = this._GetPermissionAPI();
        return new Promise((d, e) => c.requestPermission(this._MapPermissionID(c,
    ...
    _BinaryStringToBlob(b, c) {
        ...
        let e = d >> 2,
            ...
            h, k;
        for (k = h = 0; h < e; ++h) g[h] =
            b.charCodeAt(k++) | b.charCodeAt(k++) << 8 | b.charCodeAt(k++) << 16 | b.charCodeAt(k++) << 24;
...
RuntimeInterface$jscomp$1.AddDOMHandlerClass(class extends self.DOMHandler {
    constructor(a) {
        ...
        a.AddRuntimeComponentMessageHandler("runtime",
            "load-webfonts", d => this._OnLoadWebFonts(d));
        ...
        a.AddRuntimeComponentMessageHandler("runtime",
            "set-exporting-to-video", d => this._SetExportingToVideo(d));
    ...
    Attach() {
        ...
        "undefined" !== typeof PointerEvent ? (window.addEventListener("pointerdown", b => {
            ...
            b => this._OnMouseEventAsPointer("pointermove", b)), window.addEventListener("mouseup", b => this._OnMouseEventAsPointer("pointerup", b)), window.addEventListener("touchstart", b => {
            this._HandlePointerDownFocus(b);
    ...
    _OnWindowResize() {
        if (!this._isExportToVideo && this._enableWindowResizeEvent) {
            ...
            this._iRuntime.IsiOSWebView() && (-1 !== this._simulatedResizeTimerId && clearTimeout(this._simulatedResizeTimerId), this._OnSimulatedResize(a,
                b, 0))
        }
    }
    ...
    _TrySetTargetOrientation() {
        ...
        if (screen.orientation && screen.orientation.lock) screen.orientation.lock(a).catch(b => console.warn("[Construct] Failed to lock orientation: ", b));
            ...
            let b = !1;
            screen.lockOrientation ? b = screen.lockOrientation(a) : screen.webkitLockOrientation ? b = screen.webkitLockOrientation(a) : screen.mozLockOrientation ? b = screen.mozLockOrientation(a) :
    ...
    _OnMouseEvent(a, b, c) {
        this._isExportToVideo ||
            IsCompatibilityMouseEvent(b) || this._PostToRuntimeMaybeSync(a, {
                button: b.button,
                buttons: b.buttons,
                clientX: b.clientX,
                clientY: b.clientY + this._vkTranslateYOffset,
    ...
    _OnScreenReaderTextEvent(a) {
        var b = a.type;
        "create" === b ? (b = document.createElement("p"), b.id = "c3-sr-" + a.id, b.textContent = a.text, this._screenReaderTextWrap.appendChild(b)) : "update" ===
...
self.RuntimeInterface.AddDOMHandlerClass(class extends self.DOMHandler {
    ...
    _OnNavigate(a) {
        ...
        if ("back" === b) navigator.app && navigator.app.backHistory ? navigator.app.backHistory() : window.history.back();
            ...
            const c = a.target;
            a = a.exportType;
            self.cordova && self.cordova.InAppBrowser ? self.cordova.InAppBrowser.open(b, "_system") : "preview" === a || "windows-webview2" === a ? window.open(b, "_blank") : this._isConstructArcade ||
    ...
    _OnRequestFullscreen(a) {
        if ("windows-webview2" === this._exportType || "macos-wkwebview" === this._exportType) self.RuntimeInterface._SetWrapperIsFullscreenFlag(!0), this._iRuntime._SendWrapperMessage({
            ...
            a = a.navUI;
            1 === a ?
                b.navigationUI = "hide" : 2 === a && (b.navigationUI = "show");
    ...
    _OnExitFullscreen() {
        if ("windows-webview2" === this._exportType || "macos-wkwebview" ===
            this._exportType) self.RuntimeInterface._SetWrapperIsFullscreenFlag(!1), this._iRuntime._SendWrapperMessage({
            type: "set-fullscreen",
            fullscreen: !1
        });
        else {
...
self.RuntimeInterface.AddDOMHandlerClass(class extends self.DOMHandler {
    constructor(a) {
        ...
        this.AddRuntimeMessageHandlers([
            ...
            ["RequestRewardedBreak",
                this.RequestRewardedBreak.bind(this)
            ],
    ...
    NotifyGameplayStart() {
        this._pokiSDKLoaded && (this._finishedLoadingSent || console.log("Don't forget to send the Loading Finished Notification, or configure the Poki Plugin to manage it automatically."), this._gameplayActive = !0,
...
self.RuntimeInterface.AddDOMHandlerClass(class extends self.DOMHandler {
    ...
    _GetSafeAreaInset() {
        ...
        a = [a.getPropertyValue("--temp-sai-top"), a.getPropertyValue("--temp-sai-right"),
            a.getPropertyValue("--temp-sai-bottom"), a.getPropertyValue("--temp-sai-left")
        ].map(c => {
            c = parseInt(c, 10);
    ...
    async _OnRequestWakeLock() {
        if (!this._screenWakeLock) try {
            this._screenWakeLock =
                await navigator.wakeLock.request("screen"), this._screenWakeLock.addEventListener("release", () => this._OnWakeLockReleased()), console.log("[Construct] Screen wake lock acquired"), this.PostToRuntime("wake-lock-acquired")
        } catch (a) {
